...
async function sendDiscordNotification(memberData) {
  ...
  try {
    ...
    const response = await fetch(webhookUrl, {
      ...
      body: JSON.stringify({
        content:
          "📢 **New member joined Job Club!** Please welcome them to the community! 👋",
        embeds: [embed],
      }),
    });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const emailResult = await triggerEmailAutomation(
      memberProfile,
      missingItems,
    ).catch((err) => {
    ...
    const discordResult = await sendDiscordNotification(memberProfile).catch(
      (err) => {
